...
function MobileNote({
  ...
  const newNote = (newValue) => {
    if (open) {
      mutate({
        ...newValue,
        repertoireId: repertoireSelected ?? repertoires[0].id,
      });
      textFielTitledRef.current.focus();
    } else {
      mutate({
  ...
  return !isLoading ? (
    <Stack width="100%" gap={2} className="notes">
      <Typography variant="h1" fontSize={40} color="primary" textAlign="center">
        Notes
      </Typography>
      {!isLoading && repertoires.length > 0 && (
        <Box
          display="flex"
          sx={{ width: "100%", marginX: "auto", justifyContent: "center" }}
        ...
        >
          ...
          <IconButton
            onClick={(e) =>
              setOpenOption({
                open: true,
                selected: repertoires.find((x) => x.id === repertoireSelected),
      ...
      {!noteIsLoading && notes.length > 0 ? (
        ...
        <Button
          ...
          sx={{ maxWidth: "350px", marginX: "auto" }}
          variant="contained"
        >
          Ajouter une note
        </Button>
      ...
      {(noteSelected || open) && (
        <Note
          ...
          note={
            open
              ? null
              : notes.find((x) => x.id === noteSelected) ?? noteSelected
          }
          editOpen={open}
